...
void setup() {
  ...
  for (int i=0;i<numTimePoint;i++) 
    timepointflag[i]=0;  
#ifdef DEBUG
  Serial.print("PCA9685 init. PWM Frequency: ");  Serial.print(pwmfreq); Serial.println(" Hz");
#endif
...
#ifdef DEBUG
  Serial.println("mode (==0), pointx, pointy, (rectangle)");
  Serial.println("mode (==1), pointx, pointy, (triangle)");
  Serial.println("mode (==10), minPerceptVoltageThr, maxVoltage, resolution[0], resolution[1], logvalue[0], logvalue[1],");
  Serial.println("mode (==-1),");
  for (int i = 0; i < numMotor; i++) {
    Serial.print("motor "); Serial.print(i); Serial.print(": "); Serial.print(motorPoints[i][0]); Serial.print(", "); Serial.println(motorPoints[i][1]);
  }
#endif
}
...
void loop() {
  while (blSerial.available() > 0) {      // when serial data is availabe (i.e. user command)
    ...
    switch(mode) {   
      case 0:
        Serial.read();      // to empty the buffer which might have ','    
        setPin(PCA9685_SLAVEADR1, 61, 0);    // stop currently active motors
        startflag = 0;
        movieflag = 0;
        for (int i=0;i<numTimePoint;i++) 
...
#ifdef DEBUG
        Serial.println(" ");
        Serial.println("stop stimlation");
#endif
        break;
      case 1:      
        pointx = Serial.parseFloat();
        pointy = Serial.parseFloat();
        Serial.read();      // to empty the buffer which might have ','
  #ifdef DEBUG
        Serial.print("mode: "); Serial.print(mode); Serial.print(", x/y: "); Serial.print(pointx); Serial.print(", "); Serial.println(pointy);
  #endif
        pointQuantization(&pointx, &pointy, mode);
        updateMotorVoltage(pointx, pointy, mode);
        break;
      case 2:  // shoot
        Serial.read();      // to empty the buffer which might have ','  
        if (startflag==1) {
        ...
        maxVoltage = 4000;
        minPerceptVoltageThr = 2000;
        for (int i=0;i<3;i++) {
          setPin(PCA9685_SLAVEADR1,motorPlacement[13],disttovolt((float)i/2.0));
          setPin(PCA9685_SLAVEADR1,motorPlacement[14],disttovolt((float)i/2.0));
          setPin(PCA9685_SLAVEADR1,motorPlacement[9],disttovolt(1-(float)i/2.0));
          setPin(PCA9685_SLAVEADR1,motorPlacement[10],disttovolt(1-(float)i/2.0));
          delay(20);
        }
        for (int i=1;i<3;i++) {
          setPin(PCA9685_SLAVEADR1,motorPlacement[9],disttovolt((float)i/2.0));
          setPin(PCA9685_SLAVEADR1,motorPlacement[10],disttovolt((float)i/2.0));
          setPin(PCA9685_SLAVEADR1,motorPlacement[5],disttovolt(1-(float)i/2.0));
          setPin(PCA9685_SLAVEADR1,motorPlacement[6],disttovolt(1-(float)i/2.0));
          delay(20);
        }
        for (int i=1;i<3;i++) {
          setPin(PCA9685_SLAVEADR1,motorPlacement[5],disttovolt((float)i/2.0));
          setPin(PCA9685_SLAVEADR1,motorPlacement[6],disttovolt((float)i/2.0));
          setPin(PCA9685_SLAVEADR1,motorPlacement[1],disttovolt(1-(float)i/2.0));
          setPin(PCA9685_SLAVEADR1,motorPlacement[2],disttovolt(1-(float)i/2.0));
          delay(20);
        }        
        setPin(PCA9685_SLAVEADR1, 61, 0);
        maxVoltage=2458;
      ...
      case 3:  // shoot enemy
        ...
        setPin(PCA9685_SLAVEADR1, 61, 0);
        delay(50);     
        setPin(PCA9685_SLAVEADR1,motorPlacement[0],4096*0.9);
        setPin(PCA9685_SLAVEADR1,motorPlacement[1],4096*0.9);
        setPin(PCA9685_SLAVEADR1,motorPlacement[2],4096*0.9);
        setPin(PCA9685_SLAVEADR1,motorPlacement[3],4096*0.9);
        delay(70);
        setPin(PCA9685_SLAVEADR1, 61, 0);
        maxVoltage=2458;
        minPerceptVoltageThr=1229;        
        break;
      case 4:  // shot by enemy
        Serial.read();      // to empty the buffer which might have ','    
        if (startflag==1) {
          startflag=0;     
          setPin(PCA9685_SLAVEADR1, 61, 0);                            
        ...
        minPerceptVoltageThr = 2000;        
        for (int i=0;i<3;i++) {
          setPin(PCA9685_SLAVEADR1,motorPlacement[1],disttovolt((float)i/2.0));
          setPin(PCA9685_SLAVEADR1,motorPlacement[2],disttovolt((float)i/2.0));
          setPin(PCA9685_SLAVEADR1,motorPlacement[5],disttovolt(1-(float)i/2.0));
          setPin(PCA9685_SLAVEADR1,motorPlacement[6],disttovolt(1-(float)i/2.0));
          delay(20);
        }
        ...
        for (int i=1;i<3;i++) {
          setPin(PCA9685_SLAVEADR1,motorPlacement[9],disttovolt((float)i/2.0));
          setPin(PCA9685_SLAVEADR1,motorPlacement[10],disttovolt((float)i/2.0));
          setPin(PCA9685_SLAVEADR1,motorPlacement[13],disttovolt(1-(float)i/2.0));
          setPin(PCA9685_SLAVEADR1,motorPlacement[14],disttovolt(1-(float)i/2.0));
          delay(20);
        }        
        setPin(PCA9685_SLAVEADR1, 61, 0);
        delay(50);
        setPin(PCA9685_SLAVEADR1,motorPlacement[12],4096*0.9);
        setPin(PCA9685_SLAVEADR1,motorPlacement[13],4096*0.9);
        setPin(PCA9685_SLAVEADR1,motorPlacement[14],4096*0.9);
        setPin(PCA9685_SLAVEADR1,motorPlacement[15],4096*0.9);
        delay(70);
        setPin(PCA9685_SLAVEADR1, 61, 0);
        maxVoltage=2458;
        minPerceptVoltageThr=1229;
      ...
      case 5:  // up
        ...
        setPin(PCA9685_SLAVEADR1,motorPlacement[1],2500);
        setPin(PCA9685_SLAVEADR1,motorPlacement[2],2500);
        delay(100);
        setPin(PCA9685_SLAVEADR1, 61, 0);
        break;
      case 6:  // down
        ...
        setPin(PCA9685_SLAVEADR1,motorPlacement[13],2500);
        setPin(PCA9685_SLAVEADR1,motorPlacement[14],2500);
        delay(100);
        setPin(PCA9685_SLAVEADR1, 61, 0);
        break;
      case 7:  // left
        ...
        setPin(PCA9685_SLAVEADR1,motorPlacement[4],2500);
        setPin(PCA9685_SLAVEADR1,motorPlacement[8],2500);
        delay(100);
        setPin(PCA9685_SLAVEADR1, 61, 0);
        break;
      case 8:  // right
        ...
        setPin(PCA9685_SLAVEADR1,motorPlacement[7],2500);
        setPin(PCA9685_SLAVEADR1,motorPlacement[11],2500);
        delay(100);
        setPin(PCA9685_SLAVEADR1, 61, 0);
        break;
      case 9:  // jump
      ...
      case 10:  // explosion
        ...
        for (int i=0;i<6;i++) {
          for (int j=0;j<2;j++) {
            setPin(PCA9685_SLAVEADR1,motorPlacement[randpattern[j+randpatternNum][i]],4096*0.9);
          }
          delay(80);
          setPin(PCA9685_SLAVEADR1, 61, 0);                  
          delay(20);
        ...
        if (randpatternNum==4)
          ...
          randpatternNum=randpatternNum+2;
        break;
      case 11:  // CW rotation
        Serial.read();      // to empty the buffer which might have ','      
        CWflag = 1;
        pointx = circlePts[0][0];
        pointy = circlePts[1][0];
    ...
    if (dtMillis >= dt) {
      ...
      if (movieflag) {
        if ((!timepointflag[0])&&((previousMillis-startMillis)>TIMEPOINT1)) {
          // 7450
          // 80
          setPin(PCA9685_SLAVEADR1,motorPlacement[5],4096*0.4); 
          setPin(PCA9685_SLAVEADR1,motorPlacement[6],4096*0.4);
          setPin(PCA9685_SLAVEADR1,motorPlacement[9],4096*0.4);
          setPin(PCA9685_SLAVEADR1,motorPlacement[10],4096*0.4);
          delay(300);
          ...
          // 10450
          setPin(PCA9685_SLAVEADR1,motorPlacement[5],4096*0.7); // 140
          setPin(PCA9685_SLAVEADR1,motorPlacement[6],4096*0.7);
          setPin(PCA9685_SLAVEADR1,motorPlacement[9],4096*0.7);
          setPin(PCA9685_SLAVEADR1,motorPlacement[10],4096*0.7);        
          delay(400);
          ...
          // 16400
          setPin(PCA9685_SLAVEADR1,motorPlacement[5],4096*0.55); // 100
          setPin(PCA9685_SLAVEADR1,motorPlacement[6],4096*0.55);
          setPin(PCA9685_SLAVEADR1,motorPlacement[9],4096*0.55);
          setPin(PCA9685_SLAVEADR1,motorPlacement[10],4096*0.55);
          delay(400);
          ...
          for (int i=0;i<20;i++) {
            for (int j=0;j<3;j++) {
              setPin(PCA9685_SLAVEADR1,motorPlacement[randpatternMovie[j][i]],4096*0.5); // 100
            }
            delay(80);
            setPin(PCA9685_SLAVEADR1, 61, 0);                  
            delay(120);          
  ...
  //          setPin(PCA9685_SLAVEADR1,motorPlacement[7],disttovolt((float)i/10.0));
          ...
          // 0,1,2
          for (int i=0;i<3;i++) {
            setPin(PCA9685_SLAVEADR1,motorPlacement[0],1000+100*i);
            setPin(PCA9685_SLAVEADR1,motorPlacement[1],1000+100*i);
            setPin(PCA9685_SLAVEADR1,motorPlacement[2],1000+100*i);        
            delay(50);  
          }
          ...
          // 70750
          maxVoltage=3277; // 131
          minPerceptVoltageThr=2458; // 98 (200)    
          // 13,14
          // 9,10
          for (int i=0;i<7;i++) {
          ...
          // 76130
          setPin(PCA9685_SLAVEADR1,motorPlacement[8],4096*0.7); // 115
          setPin(PCA9685_SLAVEADR1,motorPlacement[12],4096*0.7); // 115
          delay(200);
          // 76330
          setPin(PCA9685_SLAVEADR1, 61, 0);        
          delay(420);
          ...
          // 76750
          setPin(PCA9685_SLAVEADR1,motorPlacement[11],4096*0.7); // 115
          setPin(PCA9685_SLAVEADR1,motorPlacement[15],4096*0.7);        
          delay(200);
          ...
          // 77350
          setPin(PCA9685_SLAVEADR1,motorPlacement[1],4096*0.7);
          setPin(PCA9685_SLAVEADR1,motorPlacement[2],4096*0.7);        
          setPin(PCA9685_SLAVEADR1,motorPlacement[5],4096*0.7);
          setPin(PCA9685_SLAVEADR1,motorPlacement[6],4096*0.7);        
          delay(250);
...
#ifdef DEBUG
  Serial.print("quantized x/y: "); Serial.print(*ptx); Serial.print(", "); Serial.println(*pty);
  Serial.print("block index x/y: "); Serial.print(blockidx[0]); Serial.print(", "); Serial.println(blockidx[1]);
#endif
}
